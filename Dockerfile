...
CMD ["sh", "-c", "PORT=9090 node server/websocket-server.js & npm run preview -- --host 0.0.0.0 --port 3000"]
...
CMD ["sh", "-c", "PORT=9090 node server/websocket-server.js & npm run dev -- --host 0.0.0.0 --port 5173"]
