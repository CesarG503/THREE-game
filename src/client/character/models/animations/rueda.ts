...
export class StylizedAnimationSystem implements IAnimationSystem {
    ...
    update(parts: LimbParts, state: AnimationState, model: any) {
        ...
        if (isMoving && !isSuperman && isGrounded) {
            ...
            // Small hops visually when moving
            hopAmount = state.isRunning
                ? Math.abs(Math.sin(walkTime)) * 0.12
                : Math.abs(Math.sin(walkTime)) * 0.05;
        } else if (!isMoving) {
            // Subtle sway during idle breathing
            headSwayZ = Math.sin(timeSec) * 0.012;
        }

        const targetHeadY = 1.3 - crouchOffset + (isMoving ? bounceAmount : breatheBob) + hopAmount;
        const targetBodyY = 1.3 - 6 * pixelScale - crouchOffset + (isMoving ? bounceAmount * 0.75 : breatheBob * 0.5) + hopAmount;

        let targetRArmX = 4 * pixelScale + 2 * pixelScale;
        let targetLArmX = -4 * pixelScale - 2 * pixelScale;
        let targetRArmY = 1.3 - 2 * pixelScale - crouchOffset + (isMoving ? bounceAmount * 0.75 : 0) + hopAmount;
        let targetLArmY = 1.3 - 2 * pixelScale - crouchOffset + (isMoving ? bounceAmount * 0.75 : 0) + hopAmount;
        let targetRArmZ = 0;
        let targetLArmZ = 0;
        ...
        let targetLegY = baseLegY - legCrouchOffset + hopAmount;
        ...
        if (isSuperman) {
            ...
            if (isJumpingOrFalling) {
                ...
                // Continuous circular rotation (turbine/wheel effect)
                baseRLegX = walkTime;
                baseLLegX = walkTime + Math.PI;
                ...
                const swingScale = state.isRunning ? 1.35 : 0.95;
                baseRArmX = Math.sin(walkTime) * swingScale;
                baseLArmX = -Math.sin(walkTime) * swingScale;
