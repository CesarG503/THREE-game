...
export class LevelBuilder {
    ...
    build() {
        this.addStaticBox(0, -0.5, 0, 100, 1, 100, 0x333333);
        this.addStaticBox(5, 1, 0, 4, 2, 4, 0x4488ff);
        ...
        this.addStaticBox(10, 5, 10, 4, 10, 4, 0x888888);

        const ladder = new Ladder(this.scene, this.world, {
            position: new THREE.Vector3(7.8, 0, 10),
            height: 10,
            width: 1.2,
            rotationY: Math.PI / 2
        });
    ...
    addStaticBox(x: number, y: number, z: number, w: number, h: number, d: number, color: number, rotation: any = { x: 0, y: 0, z: 0 }) {
