export class ChatManager {
    ...
    openChat() {
        ...
        const chatInputContainer = document.getElementById("chat-input-container") as HTMLElement | null;
        const chatInput = document.getElementById("chat-input") as HTMLInputElement | null;
        const chatHint = document.getElementById("chat-hint") as HTMLElement | null;

        if (!chatInputContainer || !chatInput || !chatHint) return;
    ...
    addChatMessage(playerId: any, playerName: any, message: string) {
    ...
    getPlayerColor(playerId: any) {
        const colors = ["#4488ff", "#ff4444", "#44ff44", "#ffff44", "#ff44ff", "#44ffff", "#ff8844", "#8844ff"];
        const hash = playerId.split("").reduce((acc: any, char: any) => acc + char.charCodeAt(0), 0);
