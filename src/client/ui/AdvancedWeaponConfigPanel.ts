...
export class AdvancedWeaponConfigPanel {
    ...
    init() {
        ...
        this.container.querySelector('#awc-reset-btn').onclick = () => {
            if (this.weapon && this.weapon.originalConfig) {
                ...
                this.drawTrajectory();
                this.init3DPreview();
                const cm = this.getConstructionMenu();
                if (cm) {
                    cm.currentDraftItem = draft;
                    cm.selectItem(draft);
                }
            }
        };
    ...
    populateProperties() {
        ...
        const createInput = (label, type, value, key) => {
            ...
            // Instantly sync value while typing and after select/checkbox changes.
        ...
        if (createSection('EFECTOS')) {
            ...
            createInput('Efecto VFX al Colisionar', vfxOptions, this.weapon.tracerCollisionVFX || 'Ninguno', 'tracerCollisionVFX');
        }
    ...
    getConstructionMenu() {
        return this.constructionMenu || this.game?.constructionMenu || this.game?.uiManager?.constructionMenu || null;
    }

    syncToOriginalMenu(key = null, value = undefined) {
        const cm = this.getConstructionMenu();
        if (!cm) return;
        if (this.weapon) cm.currentDraftItem = this.weapon;
        ...
        const syncOne = (prop) => {
            ...
            if (!input) return;
            const nextValue = prop === key && value !== undefined ? value : this.weapon?.[prop];
            if (input.type === 'checkbox') input.checked = Boolean(nextValue);
            else input.value = nextValue !== undefined ? nextValue : '';
        };

        if (key) syncOne(key);
        else Object.keys(map).forEach(syncOne);
    }
    ...
    hide() {
        ...
        if (this.game?.cameraController && !this.getConstructionMenu()?.isVisible) {
